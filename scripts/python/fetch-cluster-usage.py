...
LEGEND_TEXT = """# Cluster usage report — legend

Generated by scripts/python/fetch-cluster-usage.py.

## Files in this folder
- `resources.csv` — CPU/memory configured vs. real usage, one row per scope (see `level`).
- `ooms.csv`      — one row per OOM-killed container.
- `report.json`   — the same data nested (namespace → workload → pod → container) plus rollups.
- `by-stage/<stage>/` — (in the top-level folder) the same files restricted to one stage.

## resources.csv — the `level` column says what each row aggregates
- `cluster`   — grand total across every namespace in the report.
- `stage`     — total across all namespaces of one stage (ref/test/prod/phase/pnext/other);
                `stage` is set, `namespace` is empty.
- `namespace` — total for one namespace.
- `workload`  — one Deployment / StatefulSet / DaemonSet (pods grouped by ownerReferences).
- `pod`       — one pod (sum of its containers).
- `container` — one container (the finest level).

The `cluster` and `stage` rollup rows come first, then the per-namespace detail rows.
A `workload` row with `pod_count` = 0 is *declared but idle* (e.g. a StatefulSet scaled
to 0): its limits are the pod template's configured values and usage cells are blank.

## resources.csv columns
- `level`, `stage`, `namespace`, `workload_kind`, `workload`, `pod`, `container`
    — identity of the row (blank where not applicable to that level).
- CPU columns are in **cores**; memory columns are in **bytes**.
- `cpu_request` / `cpu_limit` / `mem_request` / `mem_limit`
    — configured requests/limits from pod specs. Blank = unset/unlimited somewhere in scope.
- `cpu_now` / `mem_now`   — usage at report time (CPU = rate over 5m; memory = working set).
- `cpu_peak` / `mem_peak` — peak over the lookback window (--window).
- `cpu_avg`               — average CPU over the window.
- `cpu_peak_util_pct` / `mem_peak_util_pct` — peak ÷ limit, as a percent (blank when no limit).
- `oom_count`             — OOM-killed containers counted in the scope.
- `pod_count` / `container_count` — how many pods/containers the row aggregates.
Blank usage cells mean Thanos had no data (or --no-thanos / Thanos was unreachable).

## ooms.csv columns
- `stage`, `namespace`, `pod`, `container` — which container OOMed.
- `source`        — `live` (current pod state), `thanos` (historical), or `both`.
- `oom_events`    — OOM-kill count over the window (from Thanos).
- `restart_count`, `exit_code` (137 = OOMKilled), `finished_at` — from live pod state.
"""
